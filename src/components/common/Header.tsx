  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-black bg-opacity-90 backdrop-blur-md border-b border-zinc-800 px-6">
      <nav className="max-w-full mx-auto py-4 flex items-center justify-between">
        <Link href="/" className="text-2xl font-bold text-white pl-7" style={{ fontFamily: "'Axion Kong', sans-serif", letterSpacing: '5px' }}>
          SIMO MOTSA
        </Link>
        {/* desktop links */}
      ...
      {open && (
        <div className="md:hidden bg-black bg-opacity-90 backdrop-blur-sm border-t border-zinc-800">
          <div className="flex flex-col px-6 py-4 space-y-4">
            <Link href={`${base}/direction`} className={`transition ${isActive(`${base}/direction`) ? 'text-white border-b-2 border-white pb-1' : 'text-zinc-400 hover:text-white'}`} onClick={() => setOpen(false)}>
              DIRECTION
            </Link>
            <Link href={`${base}/editorial`} className={`transition ${isActive(`${base}/editorial`) ? 'text-white border-b-2 border-white pb-1' : 'text-zinc-400 hover:text-white'}`} onClick={() => setOpen(false)}>
              EDITORIAL
            </Link>
            <Link href={`${base}/vfx`} className={`transition ${isActive(`${base}/vfx`) ? 'text-white border-b-2 border-white pb-1' : 'text-zinc-400 hover:text-white'}`} onClick={() => setOpen(false)}>
              VFX
            </Link>
            <Link href={`${base}/production`} className={`transition ${isActive(`${base}/production`) ? 'text-white border-b-2 border-white pb-1' : 'text-zinc-400 hover:text-white'}`} onClick={() => setOpen(false)}>
              PRODUCTION
            </Link>
            <Link href={`${base}/contact`} className="px-6 py-2 bg-white text-black rounded-full font-medium hover:bg-zinc-200 transition" onClick={() => setOpen(false)}>
              CONTACT
            </Link>
          </div>
        </div>
